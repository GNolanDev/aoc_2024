...
fn main() {
    ...
    // function to take a vector of tuples of ints (coordinates), compare each set of coordinatws with every other set, add an antinode ('#') to the antinode_grid the same number of places in both directions beyond each coordinate
        ...
        for (i, coord) in coords.iter().enumerate() {
            for (j, other_coord) in coords.iter().enumerate() {
                if i != j {
                    ...
                    // first antinode
                    let x1_antinode = x1 - x_diff;
                    let y1_antinode = y1 - y_diff;
                    // second antinode
                    let x2_antinode = x2 + x_diff;
                    let y2_antinode = y2 + y_diff;
                    // add antinodes to the antinode_grid
                    if x1_antinode >= 0 && x1_antinode < width && y1_antinode >= 0 && y1_antinode < height {
                        antinode_grid[y1_antinode as usize][x1_antinode as usize] = '#';
                    }
                    if x2_antinode >= 0 && x2_antinode < width && y2_antinode >= 0 && y2_antinode < height {
